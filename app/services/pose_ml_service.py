...
# -------------------------
# Fast Path: Pre-computed Features from Frontend
# -------------------------

def predict_from_features(features: dict) -> dict:
    """
    Fast inference path when the frontend has already extracted pose features
    using MediaPipe WASM running in the browser.

    Skips video upload, OpenCV frame extraction, and MediaPipe processing.
    Only runs StandardScaler + Random Forest — completes in milliseconds.

    features dict must contain the same 8 keys used during training.
    """
    X = np.array([[
        features["fidget_score"],
        features["sway_x"],
    ...
    # Same subtype logic as predict_from_video_file
    if prob < 0.4:
        subtype = "Low Risk (Control)"
    elif prob > 0.8:
        subtype = "Combined (High Risk)"
    elif features["fidget_score"] > 0.05:
        subtype = "Hyperactive-Impulsive"
    else:
        subtype = "Inattentive"

    return {
        "adhd_score": round(prob * 10, 2),
        "adhd_probability": round(prob, 4),
        "subtype": subtype,
        "derived_features": features,
    }